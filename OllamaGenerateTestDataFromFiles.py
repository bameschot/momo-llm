...
from ollama import GenerateResponse, chat, generate
from ollama import ChatResponse
from pathlib import Path

from Data import TOKENIZER_PROCESSED_DATA_DIRECTORY, readInputFilePaths
from DataPreProcessing import loadReplacementFile, processRawTextLineWithReplacements

CONTENT_REPLACEMENT_MARKER = '{{content}}'
SUBJECT_REPLACEMENT_MARKER = '{{subject}}'

########################################
#Parameters
########################################
parser = argparse.ArgumentParser(
        description="Generate test data with promts based on input files using a locally runnong ollama innstance"
    )
...
parser.add_argument("--prompt", type=str,default="based on the subject {{subject}} and {{content}} generate a factual conversation between 300 and 1000 words on the topic, start with 'what about about {{subject}}', what is {{subject}} or lets talk about {{subject}}, only output the conversations and no other text, put the participants names between brackets [$name], always mention {{subject}} and do not use names of participants in the conversation", help="The prompt provided to the model when generating dat, use the {{content}} marker to inejct the file contents")
...
parser.add_argument("--outputFileName", type=str,default='syntetic-conversations-test-files', help="the name of the output files")
...
parser.add_argument("--inputData", type=str,default="./input-data/wiki-simple-english-cleaned-single-articles/*.txt", help="The glob pattern for selecting input data")
parser.add_argument("--startFromFileIndex", type=int,default=10000, help="(re)start from this index in the glob file list")
...
p_startFromFileIndex=args.startFromFileIndex
...
#content = 'based on {{content}} generate a factual conversation between 300 and 2000 words on the topic given, only output the conversations and no other text, put the participants names between brackets [$name], do not use names in the conversation'
#fileSplitSizeMb = 50
...
genIdx = 0
actualGenIdx = 0
print(f'Start generating data from {inputPaths}\nwith\n- model: {p_model}\n-system prompt {p_system}\n-prompt:{p_prompt}')
for inputFilePath in inputPaths: 
    if genIdx < p_startFromFileIndex:
        print(f'skipping idx {genIdx:04d}: {inputFilePath}')
        genIdx+=1
        continue

    print(f'generating for idx {genIdx:04d}: {inputFilePath}')


    
    # response:GenerateResponse =  generate(model=p_model,system=p_system, prompt=p_prompt)
    # messageContent = response.response+'\n' 
    
    with open(inputFilePath,"r",encoding = "utf-8") as input: 
        fileContent = input.read(-1)

    subject = inputFilePath.split('/')[-1].split('.')[0].replace('_',' ')
    print(f'subject {subject}')
    
    injectedPrompt = p_prompt.replace(CONTENT_REPLACEMENT_MARKER,fileContent)
    ...
    if actualGenIdx > 0 and actualGenIdx % p_printInterval == 0:
        print(processedContent)
        print(f'----------------------------------------------------------------------------------------')
        print(f'generated {genIdx:9d}/{len(inputPaths)-p_startFromFileIndex} responses with {charactersWritten} written characters [{(time.time()-interationStartTimeS)/p_printInterval} seconds]')
        interationStartTimeS = time.time()
...
print(f'generated {actualGenIdx:9d} responses with {charactersWritten} written characters [{(time.time()-generationStartTimeS)/p_numberOfGenerations} seconds per request] in {(time.time()-generationStartTimeS)} seconds')
